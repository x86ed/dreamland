...
2. Otherwise, stage all changes (`git add -A`) and run `git commit -m "chore: <reason> checkpoint (<agent-name>)"`, where `<agent-name>` is the resolved agent identity (via `--agent-name`, then the `--hook` payload's `agent_type`/`subagent_type`, then the configured git identity, then the `janus` default). The commit's author and committer are pinned to that identity explicitly, independent of the shared `git config` value.
...
`--reason turn-complete` SHALL first check the last recorded test result (per the `pre-merge-gate` capability's test-command gating) and refuse to commit (blocking, exit 2) if tests are known-failing at the current `HEAD`. `--reason handoff` SHALL NOT gate on test results.
...
The scaffold installer SHALL bind `dreamland commit --reason turn-complete` to Claude Code's `Stop` event (alongside the existing end-of-turn commands) and `dreamland commit --reason handoff --hook` to Claude Code's `SubagentStop` event (alongside `dreamland telemetry write --tool claude-code` and the version-bump commands, per the requirement above; `dreamland coauthor` is not registered there). On platforms without a `SubagentStop`-equivalent event, only the `Stop`-bound `--reason turn-complete` invocation applies; handoff commits on those platforms rely on the same agent-driven convention described in the requirement above for `coauthor`/`telemetry write`.
