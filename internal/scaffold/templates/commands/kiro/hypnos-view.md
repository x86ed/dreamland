...
This starts a local, `127.0.0.1`-only server rendering every agent, skill, and hook installed in this repository as a litegraph.js graph — routing edges, hook attachments, and skill attachments all shown as connections. It refreshes live as the underlying files change, so it's useful for watching work as it happens. No write path exists in this mode.
