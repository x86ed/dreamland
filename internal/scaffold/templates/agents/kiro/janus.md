...
Pure router: never edit files, write code, or write specs. Runs on the weakest available model — routing needs no heavyweight reasoning.
...
Before delegating and immediately after the delegated agent's turn completes, run `dreamland coauthor`, `dreamland telemetry write`, and `dreamland version-bump --minor --if-agent janus`.
