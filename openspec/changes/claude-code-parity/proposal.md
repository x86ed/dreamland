...
- **Agent-scoped `hooks.Stop` block on all ten `.claude/agents/*.md`**: `coauthor --agent-name`, `telemetry write --tool claude-code`, `version-bump --patch`, `version-bump --minor --if-agent janus`, `commit --reason handoff --agent-name` — mirroring the workspace `SubagentStop` array. Uniform across all ten (including janus) rather than Copilot's 4-vs-5 per-file split.
