...
### Group 10 results (passive probe, interactive session, 2026-09-24; 15 events in one dispatcher session)

Source: a logging hook bound to every event in `.claude/settings.local.json` (no dreamland code involved). What the log proves, and what it does not:

- **UserPromptSubmit payload (10.1).** Fields: `cwd`, `hook_event_name`, `permission_mode`, `prompt`, `prompt_id`, `scratchpad_dir`, `session_id`, `transcript_path`. There is NO origin/source field, so no field discriminator exists. Observed prompts: a typed slash command arrives raw (`/drmlnd:morpheus complete the coding`); system prompts arrive as envelopes: `<task-notification>\n<task-id>...` (background completion), and `<agent-message from="<agent_id>">\n[Subagent hand-back] ...` (the subagent's report delivered to the dispatcher). `prompt_id` is shared by every event of a turn, including the system prompts that start a turn, so it is not a human signal. Not captured: a typed plain (non-slash) prompt, a human prompt queued while an agent runs, hook-feedback continuation, cron prompts, and the `[SYSTEM NOTIFICATION` banner (that banner is known from the caller's earlier observation, not from this log).
- **Decision 11 outcome: envelope-match discriminator, conservative.** A prompt is SYSTEM when, after trimming leading whitespace, it is empty, starts with `<agent-message`, `<task-notification`, `[SYSTEM NOTIFICATION`, or any `<tag` harness envelope (`<` followed by a letter). Anything else (including slash commands) is HUMAN and releases. The uncaptured kinds are the risk: a system prompt with an unknown envelope would release. The two-block bound and `dreamland handoff clear` remain the backstop; if a live run (12.5) shows such a prompt, add its prefix.
- **Ordering in the background case (10.2, partly proven).** For the janus dispatch: `PostToolUse(Agent)` at launch (15:43:15) -> `Stop` -> `UserPromptSubmit <agent-message>` (15:43:44) -> `SubagentStop` (15:43:46) -> `UserPromptSubmit <task-notification>` (15:44:00). The same `<agent-message>` -> `SubagentStop` -> `<task-notification>` order occurs for the earlier morpheus probe (15:40:07/08/13). Consequence: the `<agent-message>` prompt arrives BEFORE `SubagentStop` (an inject there finds no entry), and the `<task-notification>` prompt arrives AFTER it, so the notification is the correct inject point; `enforce`/`stop-check` read state at call time. Background `PostToolUse` `tool_response` carries `isAsync: true`, `status: "async_launched"`, `agentId`, `description`, `resolvedModel`, `prompt`, `outputFile`, `canReadOutputFile`. Not proven by the passive probe: that `additionalContext` from `UserPromptSubmit` reaches the dispatcher before its next action.
- **SubagentStop payload.** Fields: `agent_id`, `agent_transcript_path`, `agent_type`, `background_tasks`, `cwd`, `effort` (absent on one event), `hook_event_name`, `last_assistant_message`, `permission_mode`, `prompt_id`, `scratchpad_dir`, `session_crons`, `session_id`, `stop_hook_active`, `transcript_path`. One event (15:40:06) had an EMPTY `agent_type` with an `agent_id`; the dispatcher rule (absence of `agent_id`) treats it as a subagent (ignored by inject/enforce/stop-check/prompt) and `record` requires a non-empty `agent_type`, so it is ignored.
- **10.3 (UNVERIFIED).** A passive probe cannot show exit-2 behavior. `Stop` fired while a background agent was running (15:43:16) and the notification later started a new turn, which supports the residual-gap description in Decision 10, but `Stop` exit 2 and `PreToolUse(Agent)` exit 2 under background dispatch, and whether the notification wakes an idle dispatcher after a blocked stop, remain for group 12.
