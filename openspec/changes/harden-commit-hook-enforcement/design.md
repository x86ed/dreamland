...
**1. Hook-payload timeout: raise 200ms → 500ms, and make a timeout observably different from an empty/absent payload.**

`hookPayloadReadTimeout` becomes a `var` (not `const`) of `500 * time.Millisecond`, still bounded (never risks a real hang — Claude Code's own hook execution timeout is far larger), giving meaningfully more margin for a slow parent-process write under load (CI runners, cold-started containers, contended disks) without meaningfully slowing the legitimate case. Independently of the exact value chosen, `agentNameFromHookPayload()`'s `case <-time.After(hookPayloadReadTimeout)` branch now writes a one-line diagnostic to stderr before returning `""` — distinguishing "we waited the full timeout and nothing arrived" from "a payload arrived promptly and legitimately carried no agent identifier" (the `case res := <-ch` branch with `len(res.data) == 0` or a parse/field miss), which stays silent as it does today. This is intentionally not a hard failure: a real hook payload not arriving in time is, on today's evidence, rare and platform-timing-dependent, not a bug in dreamland's own logic — but it must stop being *indistinguishable* from the common, legitimate "no sub-agent identity applies here" case (e.g. a `Stop` payload, which never carries `tool_input.subagent_type` by design). Alternative considered: make a timeout itself blocking (exit 2) — rejected, since a hook-payload race is a platform-timing issue outside dreamland's control, and failing the whole lifecycle event over it would be disproportionate; observability (this decision) plus the exit-code contract in Decision 2 already ensures a *genuinely unresolvable* identity downstream (an unregistered candidate, or `commit`/`coauthor` unable to write git config at all) is still enforced.
...
- [Raising the hook-payload timeout to 500ms adds latency to every `coauthor`/`--if-agent` invocation that legitimately has no payload coming (the common case, e.g. plain conversational turns)] → 500ms per invocation, only on paths already bound to fire-and-forget hook events, is not perceptible at session-lifecycle granularity; the existing 200ms value already accepted this trade-off, this only widens it slightly for the observability and false-negative-reduction gained.
