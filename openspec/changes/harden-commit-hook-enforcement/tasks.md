...
## 3. Hook-payload timeout: raise and make observable

- [ ] 3.1 In `cmd/coauthor.go`, change `hookPayloadReadTimeout` from `const ... 200 * time.Millisecond` to `var hookPayloadReadTimeout = 500 * time.Millisecond`, updating its doc comment to explain the new value and that it's now a `var` so tests can override it
- [ ] 3.2 In `agentNameFromHookPayload()`'s `case <-time.After(hookPayloadReadTimeout):` branch, add `fmt.Fprintf(os.Stderr, "dreamland: hook payload not received within %s — resolving as if no agent identity was present\n", hookPayloadReadTimeout)` before `return ""`; the `case res := <-ch:` branch's empty/no-match paths remain silent, unchanged
- [ ] 3.3 Add a test asserting the timeout branch writes to stderr and the immediate-empty-payload branch does not (e.g. temporarily lower `hookPayloadReadTimeout` in the test, use an `os.Pipe()` with nothing written for the timeout case per the existing `TestAgentNameFromHookPayload_NeverBlocksWhenNoDataArrives` pattern in `cmd/coauthor_test.go`, and redirect `os.Stderr` to a pipe/buffer to capture output)
- [ ] 3.4 Confirm `cmd/version_bump.go`'s `--if-agent` path (which calls the same `agentNameFromHookPayload()`) picks up the raised timeout and new diagnostic automatically — no code change expected there, but add/extend a test confirming it
