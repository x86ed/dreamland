...
- Start algorithm (`dreamland otel-receiver`, non-foreground), all steps best-effort; the command always exits 0 so the SessionStart hook chain never fails:
  ...
  5. Health OK and `revision < ours`: verify the reported pid with `processCommandLine`/`isDreamlandReceiver` (see the safety invariant after step 6; if the check fails, treat as step 6's "leave alone" branch), then `terminateProcess(pid)`, poll dial for the port to free (up to 3 s), spawn. If the port does not free, log to stderr and exit 0.
...
- **Task 0.1 outcome (not verified).** No real Copilot OTLP export exists to inspect: no `receiver.log` from a live Copilot session was found on the development machine (the only log found is a synthetic one from a test repo, `conversation.ids=[sess-abc]`), the running receiver has never received a real export, and the code comment in `copilot.go` records that the OTel export delivered nothing in the one environment tried. Points (a) span names / `gen_ai.operation.name` values, (b) which span carries `gen_ai.usage.*`, (c) per-turn vs cumulative and nested-subagent semantics, (d) conversation id vs hook `session_id` (hook payloads do carry `session_id` UUIDs, but no span was ever compared), and (e) resource attributes are all **unverified**. The implementation follows the spec's stated assumption: count only `invoke_agent` spans, **sum** their usage. If real captures show cumulative-to-date usage, change `writeSessionUsage` from sum to max; if usage lives only on `chat` spans, change the `agentOperation` constant in `otelreceiver/receiver.go`. Task 0.1 stays open.
