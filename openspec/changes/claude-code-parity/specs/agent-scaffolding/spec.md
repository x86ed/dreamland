...
Every `.claude/agents/*.md` file **other than `janus.md`** SHALL include, in addition to `name`/`description`/`role`/`tools`/`model`, an agent-scoped `hooks:` frontmatter block using Claude Code's per-subagent hook mechanism (frontmatter `Stop:` entries, which Claude Code auto-converts to `SubagentStop` at runtime when the agent is invoked as a subagent). Shown for `nyx.md`:
...
**Superseded for `janus.md` by `deterministic-routing-and-janus-guard` (that change wins):** `janus.md` carries no `hooks.Stop` block at all, and instead an agent-scoped `hooks.PreToolUse` guard entry (`dreamland guard-router --agent-name janus`), because Janus is no longer spawned as a routing subagent and each of the five commands is redundant with the workspace-level chain for it; the statements below apply to the other nine files. Each of those nine files carries the same five commands in the same order, including `dreamland version-bump --minor --if-agent janus` on every one of them (self-filtering, silent no-op unless the invoking payload's `agent_type` is `janus` — see the `dev-workflow-hooks` capability). The value that differs per file is the `--agent-name` argument to `coauthor` and `commit`, which SHALL equal that file's own frontmatter `name:` field — `telemetry write` does not take this argument, since it has no agent-identity concept anywhere in its collector path. `--agent-name` (new CLI flag, see the `dev-workflow-hooks` capability's commit-resolution requirement) takes precedence over the existing env-var/stdin-payload resolution chain and is hardcoded here rather than left to runtime lookup: unlike the workspace-level `SubagentStop` array — one shared block that cannot know in advance which of the ten agents (or which built-in) will trigger it — a per-agent frontmatter block is declared inside that specific agent's own file, so its identity is already known at template-authoring time. This is a deliberate improvement over GitHub Copilot's equivalent mechanism, not a parity mirror: Copilot's own agent-scoped `hooks:` blocks (e.g. `hypnos.agent.md`) take no such parameter and rely on the same runtime `agent_type` stdin lookup the workspace-level path uses.
...
#### Scenario: Agent-scoped hooks block installed on every Claude Code agent except janus, each with its own agent-name argument

- **WHEN** `dreamland init` completes with "Claude Code" selected
- **THEN** each of `.claude/agents/phantasos.md`, `nyx.md`, `morpheus.md`, `phobetor.md`, `baku.md`, `iktomi.md`, `zhougong.md`, `hypnos.md`, and `mengpo.md` contains a `hooks.Stop` frontmatter entry running, in order, `dreamland coauthor --agent-name <that-file's-name>`, `dreamland telemetry write --tool claude-code`, `dreamland version-bump --patch`, `dreamland version-bump --minor --if-agent janus`, and `dreamland commit --reason handoff --agent-name <that-file's-name>`
...
- **WHEN** any `.claude/agents/*.md` file's (other than `janus.md`, which has none) `hooks.Stop` frontmatter is compared against `.claude/settings.json`'s `hooks.SubagentStop` command array
- **THEN** both list the same five underlying commands in the same order, differing only in that the frontmatter version adds `--agent-name <that-file's-name>` to the `coauthor` and `commit` invocations (`telemetry write` is unchanged on both sides — no `--agent-name` argument)
...
- `hooks:` — a map from event name to an array of `{type: command, command: "<cmd>"}` entries: `SubagentStart` runs `dreamland coauthor --agent-name <slug>`; `SubagentStop` runs `dreamland coauthor --agent-name <slug>`, `dreamland telemetry write --tool github-copilot`, `dreamland version-bump --patch`, and `dreamland commit --reason handoff --agent-name <slug>` — the same commands the workspace-level hooks file already binds to those events, plus an explicit `--agent-name` argument on `coauthor` and `commit` (the two commands that accept one; `telemetry write` has no agent-identity concept and is unchanged). `<slug>` is the agent's canonical lowercase name (`baku`, `hypnos`, `iktomi`, `janus`, `mengpo`, `morpheus`, `nyx`, `phantasos`, `phobetor`, `zhougong`) — **not** a literal copy of that file's frontmatter `name:` field, which on GitHub Copilot is a capitalized display form (`Baku`, `Meng Po`, `Zhou Gong`, etc.). The lowercase slug is used because it's what `agentNameFromHookPayload` actually extracts from a live `agent_type` field and what Claude Code's own `name:` field already is verbatim — using the display-cased form here would make `--agent-name`'s output diverge in casing from the same commands' runtime-resolved fallback, breaking the "same commit shape across platforms" goal this value exists to serve. `--agent-name` (see the `dev-workflow-hooks` capability) takes precedence over the existing runtime `agent_type`-from-payload lookup and is hardcoded here rather than left to that lookup: a per-agent frontmatter block is declared inside that specific agent's own file, so its identity is already known at template-authoring time, unlike the workspace-level file's single block shared across all ten agents. This is belt-and-suspenders, not a different behavior: agent-scoped hooks are a real, separate VS Code mechanism (distinct schema location from the workspace file, same event names and command-entry shape) that requires the `chat.useCustomAgentHooks: true` setting — which `dreamland init` writes to `.vscode/settings.json` — to fire at all; the workspace-level file has no such gate. Declaring both means hooks still fire even if a user's environment has the agent-scoped preview flag off (workspace file) or if the workspace-file mechanism is ever restricted (agent-scoped, once the flag is on).
...
- **WHEN** any of the ten `.github/agents/*.agent.md` files is installed
- **THEN** its `hooks:` frontmatter field declares `SubagentStart` running `dreamland coauthor --agent-name <slug>`, and `SubagentStop` running `dreamland coauthor --agent-name <slug>`, `dreamland telemetry write --tool github-copilot`, `dreamland version-bump --patch`, and `dreamland commit --reason handoff --agent-name <slug>`
- **AND** `<slug>` is that agent's canonical lowercase name (e.g. `mengpo`, `zhougong`), not the capitalized display form in that file's frontmatter `name:` field (e.g. `Meng Po`, `Zhou Gong`), so the five agent-scoped blocks are identical in structure but not byte-identical to each other
