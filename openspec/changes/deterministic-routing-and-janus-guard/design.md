...
- `dreamland oneiroi seed` edits the *installed* `janus.*` files (`internal/oneiroi/routing.go`, `AddStubEdge`), not the templates. `dreamland init` re-renders the installed file from the template, so anything seed-added to a live file that the template does not know about is lost on re-init. The `Agent(...)` list cannot be handled the same way (Decision 6).
...
- The `Agent(...)` list is generated by `dreamland init` from the registry, with `seed`/`fork`/`mengpo` patching the installed file for immediacy. Generating at install time is what makes it survive re-init: `dreamland init` re-renders the installed `janus.md` from the template, so a value that only `seed` had patched in would be lost. Whether the existing routing-table stub edge survives a re-init was not checked here; if it does not, that is a pre-existing gap this change does not widen (task 4.4 records the answer). A drift test ties registry, `Agent(...)`, and routing tables together so a missed touchpoint fails the build.
