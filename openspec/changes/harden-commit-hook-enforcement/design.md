...
- **The file exists, but its `head_sha` doesn't match the repository's current `HEAD`** — a stale record, not a missing one. This *is* the one deliberate fail-open branch in this design: `commit` proceeds normally. Staleness here is genuinely ambiguous (we cannot tell whether tests would still fail on the current tree) in a way outright absence is not, and the two commands run sequentially within the same turn with no intervening commits in the current hook wiring, so a mismatch should be rare and is itself observable in `.dreamland/last-test-result.json` if debugged.
