...
- Rewrite `README.md` to walk through, in order:
  ...
  - **Workflows**: the OpenSpec change lifecycle (`/opsx:propose` → `/opsx:apply` → `/opsx:archive`) and the five paths Janus can dispatch into — TDD/BDD (`nyx`→`morpheus`→`phobetor`→`baku`), Standard SDD (`phantasos`→`morpheus`→`phobetor`→`baku`), Walkabout (`iktomi`→context-dependent hop→`baku`), Tuning (`zhougong` report), and Agent Building (creation: `zhougong`→`phantasos`→`hypnos`→`phobetor`→`baku`; deletion: `zhougong`→`phantasos`→`mengpo`) — and the rule that agents hand off directly to each other without returning to Janus.
