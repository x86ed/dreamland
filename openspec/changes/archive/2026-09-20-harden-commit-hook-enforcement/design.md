...
- When a test command is configured but `commit` finds no recorded result at all for the current turn, that is treated as a broken invariant worth investigating (routed to `iktomi`), not silently treated the same as "no tests configured."
...
Rather than relying on Claude Code's `Stop` hook array halting `commit` because `test` exited 2 earlier in the same array (undocumented, and the two run as separate OS processes regardless), `dreamland test` writes `.dreamland/last-test-result.json` — `{"status": "pass"|"fail", "head_sha": "<git rev-parse HEAD>", "written_at": "<RFC3339>"}` — every time it actually runs the configured test command (not on any of its existing skip branches). `dreamland commit` reads that file before staging/committing: if `status == "fail"` and `head_sha` matches the repository's current `HEAD`, `commit` refuses to create the commit (`Blocking(...)`, changes remain staged/unstaged for the next successful run to pick up) instead of checkpointing over a known failure.

Three distinct "no usable pass record" cases, not one:

- **No test command configured at all** (`cfg.TestCommand == ""`, or the project's language/matching-files skip branches in `runTest`) — no result file was ever expected. `commit` proceeds normally, silently, exactly as today; there is nothing to flag because no expectation was ever set.
- **A test command *is* configured, but `.dreamland/last-test-result.json` doesn't exist at all** when `commit` goes to check it — under this design, `runTest` writes that file on *every* actual run (task 6.3), so its total absence despite a configured test command means the intended `test`-then-`commit` sequence didn't happen as expected: `test` never ran before `commit` in this turn, crashed before writing, or the file was deleted out from under it. This is not "no signal," it's a broken invariant — `commit` returns `Blocking(...)` with a message naming the missing path and explicitly instructing that this should route to `iktomi` to investigate why (dreamland's own commit-hook plumbing is a general-troubleshooting condition, not an OpenSpec-workflow one, matching `iktomi`'s existing role as the catch-all for exactly this kind of no-proposal/no-spec-scenario problem per the `janus` routing table). The commit does not proceed.
- **The file exists, but its `head_sha` doesn't match the repository's current `HEAD`** — a stale record, not a missing one. This *is* the one deliberate fail-open branch in this design: `commit` proceeds normally. Staleness here is genuinely ambiguous (we cannot tell whether tests would still fail on the current tree) in a way outright absence is not, and a mismatch is itself observable in `.dreamland/last-test-result.json` if debugged.

**Addendum, found dogfooding this exact mechanism immediately after implementation**: the paragraph above originally also claimed "the two commands run sequentially within the same turn... so a mismatch should be rare" as justification for treating a missing record as safe to investigate-rather-than-block. That assumption was false in practice, not just in theory — on the very first live `Stop` hook run against the rebuilt binary, `dreamland commit` observed no result file at all (the task 7.2 "route to iktomi" branch) moments before `dreamland test` (a separate process bound as a Stop-hook sibling) finished and wrote a `"fail"` record for the same `HEAD`. `commit`'s fast path (read one small JSON file, exit) evidently completed before `test`'s slower path (fork/exec a real subprocess) did, meaning Claude Code's `Stop` hook array dispatches its sibling commands without a documented sequential guarantee — the same kind of undocumented-hook-array-semantics risk Decision 2 already declined to depend on, just encountered here too. Fixed by removing the two-process structure entirely rather than trying to out-guess the scheduling: see the `test-and-commit` addition below.

Alternative considered (from the audit): rely on exit-2 short-circuiting once Decision 2 lands — rejected as the primary mechanism because it's coupled to unverified Claude Code hook-array orchestration semantics; Decision 2's exit-2 signal is kept anyway (it still tells Claude Code the turn isn't really "done" when `test` itself fails), but it is not what actually prevents the commit — the state file is. Alternative considered for the missing-but-configured case: fold it into the same fail-open treatment as a `head_sha` mismatch (both are "the record doesn't describe the current tree") — rejected, since a *stale* record at least proves `test` ran successfully at some point on some tree, while total absence proves it may never have run at all in this turn; conflating the two hides exactly the failure mode (`test` silently not running before `commit`) this whole change exists to catch.
...
**Rebase note (2026-09-20), superseding the "reads only the persisted git identity" wording above.** After this decision was implemented, the archived `fix-claude-code-handoff-integrity` refined it, and the delta spec for `dreamland commit auto-commits...` was rebased onto main so archiving this change cannot revert that work. The persisted git identity is now the third rung, not the only source: `--agent-name`, then (with `--hook`) the payload's `agent_type`/`subagent_type` when registered, then `git config --local user.name`, then `coauthor`'s resolution; author and committer are pinned with `git -c user.name=... -c user.email=...` so subject and author agree even if the shared value changes mid-commit; a handoff git failure is non-blocking (exit 1) while a turn-complete failure blocks (exit 2); "nothing to commit" from a concurrent session is a benign no-op. The `coauthor` delta likewise keeps main's Claude Code `SubagentStop` `agent_type` bullet and scenario. Nothing in this change's intent is dropped: gating, `test-and-commit`, the `--hook` stdin gating, and exit-code semantics are unchanged. `deterministic-routing-and-janus-guard` MODIFIES the same commit requirement afterwards (adding the per-session record rung and `--hook` on the `Stop` binding), so this change must archive first.
