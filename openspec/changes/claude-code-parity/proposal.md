> **Superseded in part by `deterministic-routing-and-janus-guard` (user decision, 2026-09-20; that change wins where they conflict).** Amended below: Janus's `tools:`/`"agent"` assertions, the five-command `Stop` block on all ten agents, and the rejection of the `Agent` grant/Janus as main thread. Everything else in this change stands unchanged, and neither change needs to archive first.
...
| `tools:` includes `agent` | Present on all ten | Absent on the nine non-router agents. **`janus.md` superseded:** `Agent(<registered roster>)` per `deterministic-routing-and-janus-guard` | Not mirrored for the nine non-router agents: granting it would let agents dispatch directly instead of through the main thread. For Janus, the original "rejected, no exceptions" is superseded (`claude --agent janus` was left with no dispatch capability and did the work itself); see `deterministic-routing-and-janus-guard` design.md, Decision 4. |
...
- **`--agent-name <name>` flag** on `coauthor` and `commit` — the two commands that actually resolve agent identity — takes precedence over the existing env/stdin `agent_type` lookup. Applied to both platforms' per-agent hook templates. Not a Copilot-parity item — Copilot doesn't do this either today. (Considered adding it to `telemetry write` too; dropped — that command has no agent-identity concept anywhere in its collector path or `SnapshotResult`, so the flag would be a no-op there.)
- **Agent-scoped `hooks.Stop` block on the nine non-router `.claude/agents/*.md`** (was: all ten; `janus.md` superseded by `deterministic-routing-and-janus-guard`, which replaces its `Stop` block with an agent-scoped `PreToolUse` guard): `coauthor --agent-name`, `telemetry write --tool claude-code`, `version-bump --patch`, `version-bump --minor --if-agent janus`, `commit --reason handoff --agent-name` — mirroring the workspace `SubagentStop` array. Uniform across all ten (including janus) rather than Copilot's 4-vs-5 per-file split.
...
- **New capability `fixed-pipeline-enforcement`**: `dreamland guard-artifact`, a `PreToolUse(Write|Edit)` hook enforcing `phantasos`→specs, `hypnos`→agent templates, `zhougong`→reports. ~~Janus is untouched (`Read`, `Bash`, no exceptions).~~ Superseded: `deterministic-routing-and-janus-guard` gives Janus `Read, Bash, Agent(<registered roster>)` and a `PreToolUse` guard; `guard-artifact` itself is unchanged. Two accepted, unsolved gaps: Bash-mediated writes (`mengpo`'s `rm`) and main-thread-direct edits with no subagent active.
...
- `internal/scaffold/templates/agents/claude-code/*.md` (all ten) — add `hooks.Stop` block (nine agents after supersession; `janus.md` gets a `PreToolUse` guard instead). No `tools:` changes to the nine (`janus.md`'s `tools:` is changed by `deterministic-routing-and-janus-guard`). `janus.md`, `iktomi.md` additionally get instruction-body edits (guardrail language, Phobetor routing).
...
- `cmd/coauthor.go`, `cmd/commit.go` — add `--agent-name` flag; `commit.go` also gets the `agentNameFromHookPayload()` fallback; `coauthor.go` gains the second coding-tool `Co-authored-by:` trailer.
