...
- [ ] 3.1 Add a `--hook` bool flag to `coauthorCmd` (`cmd/coauthor.go`), documented as: set only by dreamland's own hook-binding templates, never passed by a human invoking the command manually; it gates whether stdin is read for a hook payload at all in default (non-`--trailer`) mode
...
- [ ] 7.2 In the same location, if `readLastTestResult` returns `nil` (no result file exists at all) **and** `cfg.TestCommand != ""` (a test command is configured for this project), return `Blocking(...)` with a message stating that a test command is configured but no result was recorded for this turn, and explicitly instructing to route to `iktomi` to investigate why `dreamland test` didn't run/record before this commit attempt — without running `git add` or `git commit`. If `cfg.TestCommand == ""`, a missing result file is expected and `commit` proceeds normally, unchanged
- [ ] 7.3 Confirm (via test) that a result file whose `HeadSHA` does not match current `HEAD` (a stale record, as opposed to a missing one) does not gate the commit — `runCommit` proceeds to its normal staging/commit behavior, per Decision 4's deliberate fail-open branch for staleness specifically
- [ ] 7.4 Add a test: with a `.dreamland/last-test-result.json` recording `"status": "fail"` at the current `HEAD` and pending changes present, `dreamland commit --reason turn-complete` creates no commit, leaves the working tree's changes staged/unstaged as before, and returns a blocking error
- [ ] 7.5 Add a test: with the same failing record but pending changes staged, running `dreamland commit --reason turn-complete` again after `.dreamland/last-test-result.json` is updated to `"status": "pass"` (still at the same `HEAD`, simulating a fix-and-rerun) succeeds and creates the commit
- [ ] 7.6 Add a test: with `cfg.TestCommand` set to a non-empty value and no `.dreamland/last-test-result.json` present at all, `dreamland commit --reason turn-complete` creates no commit and returns a blocking error whose message mentions `iktomi`
- [ ] 7.7 Add a test: with `cfg.TestCommand == ""` (or unset `cfg`) and no `.dreamland/last-test-result.json` present, `dreamland commit --reason turn-complete` proceeds and creates the commit as normal — confirming the two "missing file" cases (7.2 vs this one) are told apart correctly
