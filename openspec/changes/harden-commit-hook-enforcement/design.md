...
**1. Hook-payload reads: remove the timeout entirely — deterministic interactive-stdin detection instead of racing a clock.**

A fixed deadline can never be fully correct in either direction: too short reproduces the exact false negative the audit flagged (a legitimate payload arriving a moment late gets silently treated as absent); too long just delays the same false negative without removing it. Raising `hookPayloadReadTimeout` from 200ms to 500ms (this change's original plan) was still a guess, not a fix. The actual distinction `agentNameFromHookPayload()`/`changeSlugFromBashHookStdin()` need to make is between two cases that look identical to a blind `io.ReadAll`: (a) no payload is ever coming — the command was launched directly in an interactive terminal, the literal originally-reported bug — which must return immediately, versus (b) a real hook-invoked payload is inbound on a pipe and simply hasn't fully arrived yet, which must block until it actually does, however long that legitimately takes. No single deadline serves both correctly; the fix is to tell them apart before attempting any read, not to guess how long to wait.

Both functions gain a check, before reading: when the input is backed by an `*os.File`, `Stat().Mode()&os.ModeCharDevice != 0` is true exactly when stdin is a real terminal, and false when it's a pipe — which is how Claude Code invokes every hook command. Interactive stdin: return immediately (`""` / `"", false`), no read attempted, no goroutine, no timeout — this is the actual mechanism that prevents the originally-reported VS Code integrated-terminal hang, and it requires no timing assumption at all. Piped stdin: `io.ReadAll` synchronously to EOF with no deadline — correct because Claude Code closes its end of the pipe as soon as it finishes writing the payload (standard `exec.Cmd`/OS pipe semantics, not a timing assumption), so blocking on the real completion signal is strictly more correct than racing a clock against it, and eliminates the "payload arrived a moment too late" false-negative window a timeout could always produce.

The stderr diagnostic this change originally planned for a timeout branch is dropped along with the timeout — there is no longer a race to distinguish from a legitimate empty payload; both remaining outcomes (interactive-skip, or a promptly-read payload that legitimately carries no agent identifier, e.g. a `Stop` payload, which never carries `tool_input.subagent_type` by design) are unambiguous and need no diagnostic.

Applies uniformly to both duplicated instances: `cmd/coauthor.go`'s `agentNameFromHookPayload` and `cmd/version_bump.go`'s `changeSlugFromBashHookStdin` (currently forked, near-identical implementations sharing the same 200ms-constant-plus-goroutine-plus-`select` pattern) — `hookPayloadReadTimeout` and `bashHookStdinReadTimeout` are deleted outright, along with the goroutine/channel/`select` machinery in both, not adjusted. Alternative considered (this change's own first draft): raise the timeout to 500ms and add a timeout-vs-empty diagnostic — rejected per explicit feedback that any fixed deadline is still a guess that can be wrong in either direction, where character-device detection is a deterministic classification with no false-negative window at all.
...
- [Removing the timeout means a piped (non-interactive) stdin that is somehow never written to and never closed would now block `coauthor`/`version-bump --change-from-command` indefinitely, instead of degrading after a fixed wait] → Accepted: this requires whatever invoked the command to have opened a pipe and then neither written to it nor closed it, which is not how Claude Code's documented hook-invocation model works (it writes the full payload and closes its end promptly, same as any well-formed `exec.Cmd` parent). The actual originally-reported hang — a human running `dreamland coauthor` directly in an interactive terminal — is fully solved by the character-device check, which needs no timing assumption at all.
