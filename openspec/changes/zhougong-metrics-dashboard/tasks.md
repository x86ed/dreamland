...
- [ ] 1.3 Implement `FlowPath(runs)` and `Transitions(runs)` per design decision 3.
...
- [ ] 3.2 Views: agent calls, tokens per agent, token-to-code per run, runs per branch, flow path; compare view with deltas and flow diff.
